...
- **SEQUENCEINPUT 函数**：接受字符串参数，在下一次输入等待时自动作为用户输入提交
  - 调用 `SEQUENCEINPUT("输入内容")` 后，下一次进入 WaitInput 状态时，引擎调用 `PressEnterKey` 处理队列中的字符串。行为与文本框输入并按回车键一致
  - 字符串中的 `\n` 会被拆分为多段，每段分别送入一个 ERB WaitInput
