...
### ![](../assets/images/IconSK.webp)`RAND` エントリーセーフガード（v12.2.0）
!!! summary ""

    RAND 乱数の引数不正時にポップアップでスクリプトを中断しなくなり、エントリーでのクランプに変更。互換モード RAND の正規化戦略に準拠。
...
!!! info "API"
    ...
    | 呼び出し形式 | 引数不正 | EM+EE | Skia版 |
    |:---|:---|:---|:---|
    | `RAND:n` | `n ≤ 0` | ポップアップで中断 | `0` を返す |
    | `RAND(max)` / `RAND(min,max)` | `max ≤ min` | ポップアップで中断 | 下限 `min` を返す |
    | 浮動小数点引数（例：`RAND(0.5, 1.0)`） | `max ≤ min` | ポップアップで中断 | 下限 `min` を返す |

!!! hint "ヒント"

    引数不正の初回トリガー時のみコンソールへエラー色の警告を一度出力し（デバッグモードではデバッグウインドウにも出力）、以降は警告なしでクランプしてループ呼び出し時のスパムを防止。エラー表示で確率ロジックの異常を検出していたスクリプトは、デバッグウインドウでの警告出力確認に置き換えてください。
...
### ![](../assets/images/IconSK.webp)変数書き込み監視
!!! summary ""

    デバッグウィンドウの変数ウォッチページに「監視開始 / 監視停止してレポート出力」ボタンを追加。ウォッチリストの変数を対象に、監視期間中の ERB スクリプトによる書き込み（変数・新値・関数・ファイル行番号）を記録する。明細は `debug\var_write.log` に出力（10 万件超過後は集計のみ）、停止時にコンソールへサマリーレポートを出力。単発代入（`=` / `+=` 等）・`VARSET` / `CVARSET`・配列リテラル代入に対応；デバッグウィンドウ自身の書き込み（ロック書き戻し/値セル代入/コンソール手動代入）は自動除外；連想配列・文字列添字は未対応で、監視開始時にスキップ理由を明示。
