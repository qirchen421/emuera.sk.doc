...
#### LOCAL  
#### LOCALS  
**※此变量已`废弃`。请考虑使用`#DIM`、`#DIMS`替代。**  
**详情请参考[用户定义的变量](user_defined_variables.md)。**  

局部变量（局部变量）。  
`LOCAL`为整数，`LOCALS`为字符串  
大小基本为`LOCAL`1000，`LOCALS`100。  
另外，可以通过`#LOCALSIZE <设置的元素数>`和`#LOCALSSIZE <设置的元素数>`  
...
与多数语言的局部变量不同，函数调用时不进行初始化。  

内部创建`LOCAL@函数名`这样的变量。  
所以对于事件函数等有多个同名函数时是共用的。另外，递归调用时会使用相同的变量。  
像`LOCAL@EVENTFIRST:10 = 567`这样调用其他函数的变量也是可能的，但不推荐（调试功能）。  
另外，调用其他函数时，如果被调用函数名包含运算符等会出错  

#### `ARG`  
#### `ARGS`  
局部变量。  
`ARG`为整数，`ARGS`为字符串  
大小基本为`ARG`1000，`ARGS`100，可通过`VariableSize.csv`更改。  
另外，自动确保仅够用的元素数，以保证函数参数定义部分正常使用。（不少于VariableSize.csv中指定的数值）  

	@FUNC002, ARG:0, ARG:1, ARG,1100  
		LOCAL = ARG:0 * ARG:1 / 100  
		RETURN LOCAL  

这种情况下，`ARG`的元素数原本是`1000`，但在`@FUNC002`中`ARG`的元素数为0～1100共1101。  
由于设想用于函数参数指定，用于其他用途可能会降低可读性。  
...
|NO         |数值  |角色＋无维|○  |○    |×  |-|-|在`CHARA**.CSV`中通过`编号,**`指定|
...
|变量名|类型|数组|赋值|保存|禁止|初始值|初始化时机|备注|
|:--|:--|:--|:--|:--|:--|:--|:--|:--|
|LOCAL|整数|一维|○  |×  |○  |-|-|#LOCALSIZE根据函数改变元素数|
|LOCALS|字符串|一维|○  |×  |○  |-|-|#LOCALSSIZE根据函数改变元素数|
