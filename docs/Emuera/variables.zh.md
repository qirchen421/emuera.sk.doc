...
| 概念 | 标识符 | 定义 | 特点 |
|------|--------|------|------|
| **角色注册编号** | `MASTER`/`TARGET`/`ASSI`/`PLAYER` | 当前游戏状态中动态分配的索引 | 从0开始连续，随角色增删而变化 |
| **角色编号** | `NO` | CharaXX.CSV中定义的固定编号 | 静态不变，用于标识角色模板 |

### 详细说明

#### 角色注册编号（动态索引）

**`MASTER`** `[VariableCode.MASTER]` — 主角的注册编号，通常为 `0`

**`TARGET`** `[VariableCode.TARGET]` — 当前调教中角色的注册编号

**`ASSI`** `[VariableCode.ASSI]` — 助手角色的注册编号，无助手时为 `-1`

注册编号的特点：
- 游戏启动时，主角自动注册，注册编号为 `0`
- 通过 `ADDCHARA` 指令注册新角色时，分配下一个连续的编号
- 通过 `DELCHARA` 指令删除角色后，后续角色的注册编号会重新排列
...
; 通过角色编号访问（静态编号）
; 使用 GETCHARA 函数获取注册编号
#DIM L_REG_NO
L_REG_NO = GETCHARA(5)      ; 获取角色编号为5的角色的注册编号
BASE:L_REG_NO:0     ; 读取该角色的基础参数0

; RELATION 是特殊情况，第二参数是角色编号而非注册编号
RELATION:TARGET:3   ; 调教中角色与角色编号3的角色的相性
```
...
REPEAT CHARANUM
    ...
    PRINTFORML 注册编号={COUNT}, 角色编号={NO:COUNT}
REND

; 查找指定角色编号的注册编号
#DIM L_FIND
L_FIND = GETCHARA(10)   ; 查找角色编号为10的角色
IF L_FIND >= 0
    PRINTFORML 找到角色，注册编号={L_FIND}
ELSE
    PRINTL 角色不存在
ENDIF
```
...
eramaker的二维数组第一参数指定角色注册编号，因此数组大小因`CHARANUM`而异。  
...
|TIME       |整数  |一维        |○  |○    |○  |-|-|-|
|MONEY      |整数  |一维        |○  |○    |○  |-|-|-|
|MASTER     |整数  |一维        |○  |○    |○  |:0 = 0|-|主角角色的**注册编号** `[VariableCode.MASTER]`，固定为0|
|TARGET     |整数  |一维        |○  |○    |×  |:0 = 1|-|调教中角色的**注册编号** `[VariableCode.TARGET]`，当前被调教对象|
|ASSI       |整数  |一维        |○  |○    |○  |:0 = -1|-|助手角色的**注册编号** `[VariableCode.ASSI]`，-1表示无助手|
|PLAYER     |整数  |一维        |○  |○    |○  |:0 = 0|-|当前执行调教的角色的**注册编号** `[VariableCode.PLAYER]`，等于MASTER或ASSI|
...
|NO         |数值  |角色＋无维|○  |○    |×  |-|-|角色编号 `[VariableCode.NO]`，在`CHARA**.CSV`中通过`编号,**`指定的**静态固定编号**，与动态分配的注册编号不同|
