...
### `font`  
	...
	<font face='～' color='～' bcolor='～' size='～'>テキスト</font>  

囲った部分のフォント、表示色、ボタンの選択中表示色、フォントサイズ、を変更します  
このタグは入れ子にすることができます  
...
- size属性 ![](../assets/images/IconSK.webp)
	フォントサイズをピクセル単位で指定します。`size='24'` または `size='24px'` のように指定します。  
	入れ子の`<font>`タグで外側のフォントサイズ設定を継承します。  
