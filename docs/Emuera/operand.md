...
- `?～#` 普通の三項演算子 優先度は=の上で他の演算子より下（判定および結果を先にまとめてから処理される）
	...
	と同じになります。
	なお、数値の三項演算子については、`()`の中に入れることにより、通常の計算の中で使うこともできますし、文字列の三項演算子については、そのまま[`PRINTFORM`](../Reference/PRINT.md)系命令に使うことができます。
	ただし、`\@～\@`形式の三項演算子では`#`を省略することもできます（省略時は偽値が空文字列となり警告が発生します）。
