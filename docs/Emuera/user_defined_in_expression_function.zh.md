...
关于"表达式中可使用的函数"中的内置函数，请参考[表达式中可使用的函数](../Reference/README.zh.md)。  
...
带有`#FUNCTION(S)`的函数不能用通常的[`RETURN`](../Reference/RETURN.zh.md)结束。而是用`RETURNF`结束。  
...
## 限制  
### 无法从CALL调用  
带有`FUNCTION(S)`标志的函数无法通过[`CALL`](../Reference/CALL.zh.md)等通常调用方式调用。  
只能在表达式中调用。  
...
### 部分指令不可使用
在带有`FUNCTION(S)`标志的函数中，不能使用[`WAIT`](../Reference/WAIT.zh.md)等伴随输入的指令、`CALL`等伴随函数调用的指令。  
使用时会出错。  
...
例如以下脚本，在[`IF`](../Reference/IF.zh.md)语句中调用`GET_ASSI_CFLAG`，在`GET_ASSI_CFLAG`中修改`ASSI`。  
...
因此此脚本依赖于评估顺序。  
不应在带有`#FUNCTION(S)`标志的函数中使用[`ADDCHARA`](../Reference/ADDCHARA.zh.md)或给`TARGET`赋值。  
可能通过调试功能被调用  
