...
> **TOシリーズ型変換関数の完全なAPIドキュメント**（TOSTR / TOSTRF / TOINT / TOFLOAT の引数シグネチャ、フォーマット文字列、サンプルコード）は、[値・型・変数 — 型変換関数](values-types.md#型変換関数)を参照のこと。本節ではFORM構文と直接関連する部分のみを扱う。
...
!!! warning "既存メカニズムの不足と改善方向"

    現在の`{}`構文は**フォーマット文字列パラメータをサポートしていない**。これが浮動小数点フォーマットの主要なギャップである：

    | 構文 | パラメータ | 欠落機能 |
    |------|-----------|---------|
    `{expr}` | 式 | — |
    `{expr,width}` | 式 + パディング幅 | — |
    `{expr,width,LEFT}` | + 揃え方向 | — |
    `{expr,width,LEFT,format}` | + フォーマット文字列 | ❌ **未サポート** |

    **実現可能な改善案**：

    1. **`TOSTRF`パラメータの拡張**（推奨、変更最小）：
       ```erb
       TOSTRF(float_value, format, width, align)
       ; 例：TOSTRF(PI, "F2", 10, "LEFT") → "3.14      "
       ```
       - 利点：字句解析器の変更不要、関数パラメータの拡張のみ
       - 欠点：`TOSTR`のパラメータパターンと不一致（TOSTR のフォーマットは第2パラメータ）

    2. **`{}`構文の拡張でフォーマット文字列をサポート**：
       ```erb
       {PI,10,LEFT,"F2"}  ; または {PI:"F2",10}
       ```
       - 利点：構文が直感的、C# の補間文字列に類似
       - 欠点：`LexicalAnalyzer.AnalyseFormattedString`と`FormatCurlyBrace`の変更が必要、影響範囲が大きい

    3. **新規`FORMATF`関数**（`STRFORM`に類似だが浮動小数点向け）：
       ```erb
       FORMATF("{0:F2}", PI)  ; → "3.14"
       ```
       - 利点：汎用性が高い、C# フォーマットを再利用可能
       - 欠点：既存の`STRFORM`と機能が重複

    **現在の推奨**：案1（`TOSTRF`の拡張）、変更が最小で既存の`TOSTRF` + `%...%`の回避策と互換性があるため。

    > **TOシリーズ関数の完全なドキュメント**：[値・型・変数 — 型変換関数](values-types.md#型変換関数)
