...
预处理行以 `#` 开头，必须紧跟 `@` 函数标签行之后、第一条指令行之前。

### 位置规则

```erb
@MY_FUNC(ARG:0, ARG:1)     ← @ 标签行
#DIM L_TEMP, 10             ← #DIM 声明（紧跟 @）
#FUNCTION                    ← #FUNCTION 声明（紧跟 #DIM）
...
!!! warning "字符串赋值的 = 和 '= 含义不同"
    ...
    | 操作符 | 含义 | 示例 |
    |--------|------|------|
    | `=` | 格式化字符串（`{变量}` 会插值） | `S = Hello {NAME}` → `Hello 艾莉娜` |
