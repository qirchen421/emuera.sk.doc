...
`BEGIN` は状態遷移の唯一の方法である。暗黙の `RETURN` を含む——`BEGIN` の後のコードは決して実行されない（**作用範囲は次節参照**）。

### `BEGIN` の作用範囲 {#begin-scope}

上記の「`BEGIN` の後のコードは実行されない」の作用範囲は、**あくまで現在の関数内**である。`BEGIN` はコールチェーン全体を中断しない：

````
@A                     @B                          エンジン
BEGIN SHOP  ←────────  CALL B            （begintype = SHOP）
（A はここで終了）      ↓
                       PRINTL この行は通常どおり実行される
                       （B が自然終了 → A の呼び出し元へ → …）
                                                    ↓ コールスタックが空に
                                                    Begin() → SHOP へ → @SHOW_SHOP
````

- `BEGIN` は `SetBegin(keyword, true)` + `state.Return(0)` にコンパイルされる：**目標状態の記録**と**現在のフレームの終了**のみで、ジャンプはしない（[Instraction.Child.cs:L3037-L3054](file:///d:/emuera/emuera.em/Emuera/Runtime/Script/Statements/Instraction.Child.cs#L3037-L3054)）。
- 呼び出し元は通常どおり実行を終える。実際の状態遷移は**コールスタックが空になった時点**で起きる（[Process.State.cs:L363-L431](file:///d:/emuera/emuera.em/Emuera/Runtime/Script/Process.State.cs#L363-L431)）。
- したがって**イベント関数は `BEGIN` が直接呼び出すものではない**：`BEGIN TURNEND` は TURNEND 状態を「予約」するだけであり、スタックが空になった後でエンジンが `@EVENTTURNEND` を呼び出す。
- `BEGIN` は **`RESULT` も書き換えない**（スクリプトの `RETURN` 文ではなくエンジンメソッドを通るため）。

!!! warning "実践上の意味"

    あるコマンドが「終了後に以降の口上／イベント処理を通したくない」ようにしたい場合、**`BEGIN` では実現できない** —— コールチェーンは通常どおり最後まで走る。明示的なスキップフラグ（本リポジトリでは `nSkipKojo` など）を使うか、そもそも重複呼び出しを書かないこと。

    **断言**：`BEGIN` は現在のフレームを1つポップするだけで、呼び出し元は実行を続ける。
    **違反時の結果**：ここで終わると誤解し、エンジンが本来呼ぶ経路上で同一処理を重複して呼ぶ ⇒ 出力が二重になる。
    **調査手順**：出力の重複 → その関数が `BEGIN` より前にエンジンのフックを自前で呼んでいないか確認 → エンジン側の同一フック呼び出し点の有無を確認。
...
| BEGIN の後のコードが実行されない | `BEGIN` は暗黙の `RETURN` を含む（作用範囲＝現在の関数） | `BEGIN` の後にコードを書かない |
| `BEGIN` がコールチェーン全体を中断すると誤解する | 呼び出し元は通常どおり実行を終え、状態遷移はスタックが空になってから | 以降の処理を飛ばしたいなら明示的なフラグ（`nSkipKojo` など）を使う |
| イベント関数は `BEGIN` が直接呼ぶと誤解する | `BEGIN` は状態を予約するだけ；スタックが空になった後にエンジンがその状態のイベント関数を呼ぶ | 前節「`BEGIN` の作用範囲」参照 |
