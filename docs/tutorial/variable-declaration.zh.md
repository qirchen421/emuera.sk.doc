...
!!! warning "#行位置规则"
    ...
    所有 `#` 开头的预处理行必须位于 `@函数名(...)` 标签行之后、第一条执行语句之前。多个 `#` 行可以连续出现，顺序不限。空行和注释行（`;`）不影响 `#` 行序列：

    ```erb
    @MY_FUNC(ARG:0)
    #DIM L_COUNT, 10          ; ✅ @ 行之后
    ; 这是注释（不影响 # 行序列）
    #DIMS L_NAME              ; ✅ 注释之后仍然有效
...
## 变量标识符：CSV／ALS／ERD { #identifiers }

声明给出数组的长度，标识符给出每个元素的名字。`HOGE:りんご` 这样的名字下标写法、`GETNUM`（名字→编号）、`ERDNAME`（编号→名字）消费的都是同一份"标识符→下标"字典，而字典的内容来自三种文件：

| 文件 | 位置 | 服务对象 |
|------|------|---------|
| `CSV/变量名.CSV`（如 `ITEM.CSV`） | `CSV` 文件夹顶层（子目录不参与） | 引擎内置变量（`ITEM`、`CFLAG`、`ABL`……） |
| `ERB/变量名.ERD` | `ERB` 文件夹（含子目录递归） | ERH `#DIM` 声明的全局变量 |
| `CSV/变量名.CSV`（与 `#DIM` 同名） | `CSV` 文件夹顶层 | 同上，与 `.erd` 地位平等 |
| 同名 `.als`（如 `HOGE.als`） | 与对应 `.erd`/`.csv` 同目录 | 别名：同一个下标的另一个名字 |
...
文件名匹配不区分大小写。文件格式与 CSV 变量文件相同，每行 `下标, 标识符`。下标超出声明的元素数时该行被警告并跳过。

### 同名 `.erd` 和 `.csv`：合并读取

同一个变量名同时存在 `.erd` 和 `.csv` 时，两个文件**合并读取**：标识符汇总进同一份字典，互补的名字都能用。
...
HOGE:りんご   ; → 0（来自 ERD）
HOGE:みかん   ; → 1（来自 CSV）
HOGE:林檎     ; → 0（与 りんご 同一个元素）
```

**只有定义了相同的标识符时才会在启动时出错并终止**（无论下标是否相同）。同一下标写不同标识符是合法的，两个名字指向同一个元素。

### 一数多名：`.als` 别名文件

单个文件内同一下标写两行时，后一行覆盖前一行（仅警告），前一个名字失效——**单个文件内没有任何写法能一数多名**。想给同一个下标起多个名字，有两条路：像上面这样分两个文件写，或者使用同名 `.als` 别名文件：
...
`HOGE.ERD` 配 `HOGE.als`，格式同样是 `下标, 别名`，一个下标可挂多个别名。别名与已有标识符冲突时静默跳过，不报错——这点与正式名互撞直接导致启动报错是有意不对称的。

### 多维数组：`@1`／`@2`／`@3` 分维命名

每一维用独立文件命名，从左边的下标开始依次对应 `1`、`2`、`3`。缺少某维文件时该维只能用数字下标：

```csv
; ERB/HOGE2D@1.ERD（第一维）
0, 赤
1, 青
```

```csv
; ERB/HOGE2D@2.ERD（第二维）
0, りんご
1, みかん
```
...
- ERD 只适用于在 ERH 中用 `#DIM` 定义的变量。`ITEM`、`CFLAG`、`ABL` 等引擎内置变量的名字只能在 `CSV` 文件夹内对应的 `ITEM.CSV`、`CFLAG.CSV` 等文件中定义，在 `ERB` 内放置同名的 `.ERD` 文件不会被读取。
- `ITEM.CSV` 的第三列（价格）是内置管线专属，用户变量的 ERD／CSV 即使写了第三列也会被忽略。

!!! warning "别依赖 `ERDNAME` 反查一数多名的结果"

    `ERDNAME(变量, 下标)` 返回的是"第一个命中的名字"，多个名字对应同一下标时反查到哪一个是不确定的。不要用它的返回值做口上分支条件。

**规格参考**：[用户定义变量](../Emuera/user_defined_variables.zh.md)（ERD 功能节）、[ERDNAME](../Reference/ERDNAME.zh.md)。
