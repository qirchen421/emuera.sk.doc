...
## 変数識別子：CSV／ALS／ERD { #identifiers }

宣言が配列の長さを決め、識別子が各要素の名前を決める。`HOGE:りんご` のような名前添字記法、`GETNUM`（名前→番号）、`ERDNAME`（番号→名前）が消費するのは同一の「識別子→添字」辞書であり、辞書の中身は3種のファイルから来る：

| ファイル | 配置 | 対象 |
|------|------|---------|
| `CSV/変数名.CSV`（例 `ITEM.CSV`） | `CSV` フォルダ直下（サブディレクトリは対象外） | エンジン内蔵変数（`ITEM`・`CFLAG`・`ABL`……） |
| `ERB/変数名.ERD` | `ERB` フォルダ（サブディレクトリ再帰あり） | ERH で `#DIM` 宣言した広域変数 |
| `CSV/変数名.CSV`（`#DIM` と同名） | `CSV` フォルダ直下 | 同上、`.erd` と対等 |
| 同名 `.als`（例 `HOGE.als`） | 対応する `.erd`/`.csv` と同ディレクトリ | エイリアス：同じ添字の別名 |
...
ファイル名の突合せは大文字小文字を区別しない。書式は CSV 変数ファイルと同じで、1行 `添字, 識別子`。宣言した要素数を超える添字の行は警告のうえスキップされる。

### 同名 `.erd` と `.csv`：マージ読み込み

同じ変数名の `.erd` と `.csv` が両方存在する場合は**マージ読み込み**となる：識別子は同一の辞書に集約され、互いに補完した名前がすべて使える。
...
**同じ識別子が定義されている場合だけ起動時にエラー吐いて終了する**（添字の一致不一致を問わない）。同じ添字に違う識別子を書くのは合法で、両方の名前が同じ要素を指す。

### 一数多名：`.als` エイリアスファイル

単一ファイル内で同じ添字を2行書いた場合、後の行が前の行を上書きし（警告のみ）、前の名前は失効する——**単一ファイル内には一数多名の書き方は存在しない**。同じ添字に複数の名前を付けたい場合は、上記のように2ファイルに分けて書くか、同名の `.als` エイリアスファイルを使う：
...
`HOGE.ERD` に対する `HOGE.als`、書式は同じく `添字, 別名`、1つの添字に複数エイリアス可。エイリアスが既存識別子と衝突した場合は警告なくスキップされ、エラーにならない——正式名同士の衝突が起動エラーになるのとは意図的に対称でない。

### 多次元配列：`@1`／`@2`／`@3` で次元ごとに命名

各次元は独立ファイルで命名し、左の添字から `1`・`2`・`3` と対応する。ファイルのない次元は数字添字のみとなる：

```csv
; ERB/HOGE2D@1.ERD（第一次元）
0, 赤
1, 青
```

```csv
; ERB/HOGE2D@2.ERD（第二次元）
0, りんご
1, みかん
```
...
- ERD は ERH 内で `#DIM` 定義した変数にのみ適用される。`ITEM`・`CFLAG`・`ABL` などのエンジン内蔵変数の名前は、`CSV` フォルダ内の対応する `ITEM.CSV`・`CFLAG.CSV` などでしか定義できない。`ERB` 内に同名の `.ERD` ファイルを置いても読み込まれない。
- `ITEM.CSV` の第3列（価格）は内蔵管線専用であり、ユーザー変数の ERD／CSV に第3列を書いても無視される。

!!! warning "`ERDNAME` の一数多名に対する逆引き結果に依存しない"

    `ERDNAME(変数, 添字)` が返すのは「最初にヒットした名前」であり、複数の名前が同じ添字に対応する場合にどれが返るかは不定である。その戻り値を口上の分岐条件に使わないこと。

**仕様参考**：[ユーザー定義の変数](../Emuera/user_defined_variables.md)（ERD 機能節）、[ERDNAME](../Reference/ERDNAME.md)。
