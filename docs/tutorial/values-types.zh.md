...
!!! skia "Skia版的溢出保护（SafeArithmetic）"

    Skia 版中，算术运算符（`+`、`-`、`*`、一元 `-`）启用了溢出保护。当运算结果超出 `long` 范围时，会输出警告并将结果钳位到 `Long.MaxValue` 或 `Long.MinValue`，而非静默回环。

    如果需要回环溢出行为（如哈希计算），请使用 [UNCHECKED 系列函数](../Reference/UNCHECKED.zh.md)：
...
| 写法 | 本质 | 说明 |
|------|------|------|
| `#DIM L_VAR` | `#DIM L_VAR, 1` | 声明长度为 1 的一维数组 |
| `L_VAR = 10` | `L_VAR:0 = 10` | `L_VAR` 是 `L_VAR:0` 的省略写法 |
| `ARG` | `ARG:0` | 内置整数参数数组的第一个元素 |
| `ARGS` | `ARGS:0` | 内置字符串参数数组的第一个元素 |
| `LOCAL` | `LOCAL:0` | 内置局部整数数组的第一个元素 |
| `RESULT` | `RESULT:0` | 内置返回值整数数组的第一个元素 |

```erb
#DIM L_VAR              ; 等价于 #DIM L_VAR, 1 — 长度为 1 的一维数组
L_VAR = 42              ; 等价于 L_VAR:0 = 42
PRINTV L_VAR            ; 输出 42
PRINTV L_VAR:0          ; 同样输出 42
```

**`:` 是数组元素访问标识符**。`变量名:索引` 表示访问该变量的第 N 个元素。

### 省略语法带来的混淆

因为 `L_VAR` 可以省略 `:0`，而 `:` 同时是数组访问的分隔符，所以当变量访问嵌套时容易出现解析歧义。

#### 反模式 1：一维数组 + 变量下标

```erb
#DIM L_IDX = 5
; ❌ 错误：FLAG 是一维数组，不支持第二维
FLAG:L_IDX:2
...
`FLAG:L_IDX:2` 的错误在于：`FLAG` 是一维数组（`FLAG:0`, `FLAG:1`, ...），但 `:L_IDX:2` 被解析为尝试访问第二维。正确的写法是用括号 `(L_IDX:2)` 将下标表达式整体包裹。
...
; 实际解析过程：
; 1. CFLAG 是角色数组，第一维省略 → 默认 TARGET
; 2. L_VAR:2 被解析为第二维的索引 → 试图访问 CFLAG:TARGET:L_VAR:2
...
; 2. TALENT 被当作变量名 → TALENT 展开为 TALENT:0（TALENT 也是角色数组，第一维默认 TARGET）
; 3. 整个式子被解析为：TALNET:(TALENT:TARGET:0):5
...
| 规则 | 说明 |
|------|------|
| **始终用括号包裹复合下标** | `FLAG:(L_IDX)` 而非 `FLAG:L_IDX` |
| **角色数组显式写第一维** | `CFLAG:TARGET:L_VAR` 而非 `CFLAG:L_VAR` |
| **警惕变量名被当作数组索引** | `TALNET:TALENT:5` 是语法正确但语义错误的代码 |
| **变量名本身就是 `:0` 省略** | 理解 `L_VAR` = `L_VAR:0` 是一切混淆的根源 |

> 这些反模式是即使有经验的开发者也会踩的坑。在编写 ERB 代码时，**遇到 `:` 分隔的多个标识符，始终停下来确认解析路径是否与意图一致**。
