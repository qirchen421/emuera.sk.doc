...
アスキーアートだったり、画像を利用していたり、色々あると思う。  
...
が、`Gamebase.csv`変数は代入不可（WINDOW_TITLEは代入可能らしい）という都合や  
...
なので実際に使用するかどうかはともあれ、どういった使い方ができるのかは知っておくと便利だと思う。  
...
『`-9223372036854775808～9223372036854775807`』という範囲らしい。  
...
小数点以下3桁目は、指定がなければ省略されるようだ。  

とりあえずそれを頭の片隅においた上で、  
実際どのように管理するかは人それぞれで、  
自分が管理しやすいように指定していいとしか言えない。  
