...
### GREPのやり方  
バグの原因を調べたり、派生元の仕組みを調べたりするのに、GREPによる検索はほぼ必須です。GREPで大抵の事はわかるのですが、上手く調べるのはなかなか難しいです。  

まず使うソフトですが、サクラエディタを使っているのであれば、検索メニューの中にGREP機能が含まれています。ちょっとした調べ物にはこれで十分ですが、大量のデータからじっくり探したい時は専用ソフトの方が便利です。  
