...
!!! info "API"
    ...
    ```
    `FOR～NEXT` 是 [`REPEAT～REND`](./REPEAT.zh.md) 的功能增强版。  
	...
	上面的两个语句功能几乎相同。  
	两者都是执行X次循环的语法，在循环过程中可以使用[`CONTINUE`](./CONTINUE.zh.md)或[`BREAK`](./CONTINUE.zh.md)。  
    ...
    如果`value`为0，则会产生无限循环，直到执行[`BREAK`](./CONTINUE.zh.md)语句才会退出。
    ...
    NEXT
    ```
    此外，如果通过 [`GOTO`](./GOTO.zh.md) 等指令直接跳转到 `FOR～NEXT` 循环内部，其行为与 `REPEAT～REND` 类似：会从进入点开始正常执行，直到 `NEXT` 指令的前一行，然后忽略 `NEXT` 指令，并从下一行继续执行。
...
### 相关项目
- [REPEAT-REND](REPEAT.zh.md)
- [WHILE-WEND](WHILE.zh.md)
