...
!!! info "API"
	...
	- `SKIPDISP` <数值>
		- 参数：`0` = 设置为不忽略
			- 非0值 = 设置为忽略
		- 说明：设置此标志后，[`PRINT`](./PRINT.zh.md)等输出将完全不会执行。
			- 此外，在此标志被设置期间，若执行到[`INPUT`及`INPUTS`](./INPUT.zh.md)指令，
    ...
    `MESSKIP` 在处于 [`WAIT`](./WAIT.zh.md) 跳过状态时返回 `1`，否则返回 `0`。
