...
!!! info "API"
    ...
    ```
	Sets selectable buttons in conjunction with the button map set by the [`CBGSETBMAPG`](./CBGSETBMAPG.en.md) command.  
	...
	For `x, y, zDepth`, see [`CBGSETSPRITE`](./CBGSETSPRITE.en.md). Note that the reference position `(x,y) = (0,0)` is the position where the screen bottom-left aligns with the image bottom-left.  
