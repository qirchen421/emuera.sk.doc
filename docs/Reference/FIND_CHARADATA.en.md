...
!!! info "API"
    ...
    ```
    Searches for files that can be targets of [`LOADCHARA`](./LOADCHARA.en.md) in the dat folder and assigns the filename (the `*` part of `chara_*.dat`) to `RESULTS`.  
    ...
    Note that `chara_.dat` (where `*` is an empty string) does not match because it cannot be specified in [`LOADCHARA`](./LOADCHARA.en.md).  
