...
!!! info "API"
    ...
    其格式与 [`PRINTS`](./PRINT.md) 指令相似，但需要指定第二个参数，即点击按钮时输入的数字或字符串。如果第一个参数中包含换行符，该换行符将被忽略且不会导致换行。
    ...
    此外，`PRINTBUTTON`命令不仅可以创建输入数字的按钮，还可以创建输入字符串的按钮。这样创建的按钮可以在执行[`INPUTS`](./INPUT.md)命令时被点击。
    ...
    - 无 - 不对齐
    - `C` - 与 [`PRINTC`](./PRINTC.md) 类似，向右对齐
    - `LC` - 与 [`PRINTLC`](./PRINTC.md) 类似，向左对齐
