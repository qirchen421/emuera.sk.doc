...
!!! info "API"
    ...
    [`HTML_PRINT`](./HTML_PRINT.md)の右揃え・左揃えバリアントです。HTMLタグを利用して描画しますが、配置位置が異なります。

    - **HTML_PRINTC**：HTML描画結果を**右揃え**で表示します。
    - **HTML_PRINTLC**：HTML描画結果を**左揃え**で表示します。

    第2引数 `cellWidth` は省略可能な整数式で、セル幅をピクセル単位で指定します。省略時は `PrintCLength * FontSize / 2` のデフォルト値が使用されます。
...
!!! example "例"
    ...
    @SYSTEM_TITLE
        ; 右揃えでHTML描画
        HTML_PRINTC "<b>右揃えテキスト</b>"

        ; 左揃えでHTML描画
        HTML_PRINTLC "<b>左揃えテキスト</b>"

        ; セル幅を指定（200ピクセル）
        HTML_PRINTC "<b>右揃えテキスト</b>", 200
