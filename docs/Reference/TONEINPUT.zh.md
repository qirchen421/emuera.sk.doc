...
!!! info "API"
    ...
    ```
	参数分别与 [`TINPUT`、`TINPUTS`](./TINPUT.md) 相同。  
	这是兼具 [`ONEINPUT`](./ONEINPUT.md) 与 TINPUT、[`ONEINPUTS`](./ONEINPUT.md) 与 TINPUTS 性质的输入接收命令。  
	使用这些命令时，即使在 Emuera 的 CONFIG 设置中启用了键盘宏功能，  
	也可能出现无法正常工作的情况，这是已知的特性。  
	此外，在 `TONEINPUTS` 中，与 [`INPUTS`](./INPUT.md) 一样，可以使用宏表达式。  
	若要在字符串中使用 `()`，请使用 `\` 进行转义。
