...
!!! info "API"
	...
	float DT_CELL_GETF dataTableName, row, columnName(, asId)
	...
	操作 `DataTable`（数据库，基于 [`DataTable`](https://learn.microsoft.com/zh-cn/dotnet/api/system.data.datatable?view=netframework-4.8) 类）单元格的函数。

	- `DT_CELL_GET`：返回 `dataTableName` 对应的 `DataTable` 中行 `row` 的列 `columnName` 的整数值。失败时返回 `0`。
	- `DT_CELL_GETF`：返回 `dataTableName` 对应的 `DataTable` 中行 `row` 的列 `columnName` 的浮点数值。失败时返回 `0.0`。
	- `DT_CELL_GETS`：返回 `dataTableName` 对应的 `DataTable` 中行 `row` 的列 `columnName` 的字符串值。失败时返回空字符串。
	...
	- `DT_CELL_ISNULL`：若名为 `dataTableName` 的 `DataTable` 的 `row` 行 `columnName` 列的值为空（不同于整数和字符串）时返回 `1`。
		...
		- 返回值为 `-2` 时，指定的行或列不存在。
	- `DT_CELL_SET`：将名为 `dataTableName` 的 `DataTable` 的 `row` 行 `columnName` 列的单元格赋值为 `value`。若省略 `value`，赋值为空。根据列的类型自动判定 `value` 的类型（整数列接受整数、浮点数列接受浮点数、字符串列接受字符串）。
		- 返回值为 `1` 时，赋值成功。
