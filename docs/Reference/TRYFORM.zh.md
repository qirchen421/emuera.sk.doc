...
!!! info "API"
    ...
    ```
	与 [`JUMP`](./JUMP.md)、[`CALL`](./CALL.md)、[`GOTO`](./GOTO.md) 相同，但可以像 [`PRINTFORM`](./PRINT.md) 一样指定函数名，即使函数不存在也不会报错。  
	`TRYJUMPFORM` 和 `TRYCALLFORM` 可以指定参数。详情请参阅[函数页面的`自定义函数中的参数指定`一节](../Emuera//function.md#_2)。  
	另外，关于使用 `TRYGOTOFORM` 直接进入循环/分支结构内部的情况，请参阅 [`TRYGOTO`](./TRY.md)、[`循环/分支结构`](../Reference/README.md#_8) 以及 [`TRYC` 系列](./TRYC.md) 的说明。
