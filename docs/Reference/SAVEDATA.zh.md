...
!!! info "API"
	...
	`SAVEDATA` 命令不会调用 `@SAVEINFO`，因此无法使用 [`PUTFORM`](./PUTFORM.zh.md) 添加注释。  
	...
	可以使用[`CHKDATA`](./CHKDATA.zh.md)命令检查是否已存在数据。  
	`SAVEDATA`命令（与[`SAVEGAME`](./SAVEGAME.zh.md)命令不同）可以在脚本的任何位置调用。

!!! info "EM+EE 扩展：存档格式增强"

    EM+EE 扩展了 `SAVEDATA` 的存档格式，在标准存档数据之后额外保存 EM 专属数据：

    - **Map 数据**：标记为 `SAVEDATA` 关键字的 [`MAP`](./MAP_MANAGE.zh.md) 字典
    - **Xml 数据**：标记为 `SAVEDATA` 关键字的 XML 文档
    - **DataTable 数据**：标记为 `SAVEDATA` 关键字的 DataTable 表

    这些数据在 [`LOADDATA`](./LOADDATA.zh.md) 加载时会被先清除再从存档恢复，确保存档切换时数据一致性。
