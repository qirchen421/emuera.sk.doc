...
!!! info "API"
	...
	- `DT_CELL_GETF`: Returns the floating-point value at column `columnName` of row `row` in the `DataTable` with `dataTableName`. Returns `0.0` on failure.
	...
	- `DT_CELL_SET`: Assigns `value` to column `columnName` of row `row` in the `DataTable` with `dataTableName`. If `value` is omitted, assigns null. The type of `value` is automatically determined based on the column type (integer columns accept integers, floating-point columns accept floats, string columns accept strings).
