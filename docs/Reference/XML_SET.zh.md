...
!!! info "API"
    ...
    1. 从以 `xmlId` 的字符串转换结果([`TOSTR`](https://zh.osdn.net/projects/emuera/wiki/exmeth#h5-str.20TOSTR.28int.20value.2C.20str.20format.20.3D.20.22.22.29))为 ID 而指定的 [`XmlDocument`](https://docs.microsoft.com/zh-cn/dotnet/api/system.xml.xmldocument?view=netframework-4.8) 中检索元素节点进行更新。若指定的 `XmlDocument` 不存在，返回 `-1`。
