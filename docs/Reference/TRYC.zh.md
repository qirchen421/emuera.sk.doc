...
!!! info "API"
    ...
    其语法与 [`IF～ELSE～ENDIF`](./IF.zh.md) 类似（区别在于即使函数存在时没有要执行的处理也可以省略）。  
    因此，当使用 [`GOTO`](./GOTO.zh.md) 等指令直接跳转到 `TRYC系列～CATCH～ENDCATCH` 内部时，其行为与 `IF～ELSEIF～ELSE～ENDIF` 类似：  
    会正常执行到 `CATCH` 或 `ENDCATCH` 的前一行，然后跳转到 `ENDCATCH` 的下一行继续执行。  
    另外，关于使用 `TRYCGOTO` 或 `TRYCGOTOFORM` 直接跳入循环或分支结构内部的情况，请参阅 [`TRYGOTO`](./TRY.zh.md) 或 [`循环·分支结构`](../Reference/README.zh.md#flow-control) 章节。
