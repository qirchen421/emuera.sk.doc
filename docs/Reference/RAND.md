...
!!! warning "Skia 拡張"
    ...
    Skia版では引数不正時にエラーポップアップで中断しません（v12.2.0）。

    - `RAND:n` で `n ≤ 0` の場合：`0` を返す
    - `RAND(max)` / `RAND(min,max)` で `max ≤ min` の場合：下限 `min` を返す（浮動小数点引数も同様）
    - 初回トリガー時のみコンソールに警告を出力し、以降は静かにクランプ

    従来は `max` が `min` 以下の場合や `RAND:0` などでエラーポップアップが表示されスクリプトが終了していました。
