...
!!! info "API"
	...
	如果加载失败，将导致错误终止。  
	请务必先使用 [`CHKDATA`](./CHKDATA.zh.md) 指令检查是否可以加载，然后再执行此操作。  
	`LOADDATA`（与 [`LOADGAME`](./SAVEGAME.zh.md) 指令不同）可以在脚本的任何位置调用。

!!! info "EM+EE 扩展：存档格式增强"

    EM+EE 扩展了 `LOADDATA` 的加载行为，在加载标准存档数据之前，会先清除当前内存中的 EM 专属数据：

    - **Map 数据**：标记为 `SAVEDATA` 关键字的 [`MAP`](./MAP_MANAGE.zh.md) 字典会被清空
