...
!!! info "API"
	...
	- `DT_CELL_GETF`：`dataTableName`に対応する`DataTable`に行`row`の列`columnName`の浮動小数点数を返します。失敗した場合、`0.0`を返します。
	...
	- `DT_CELL_SET`：`dataTableName`に対応する`DataTable`に行`row`の列`columnName`の値を`value`で代入します。`value`を省略した場合，空きで代入します。列の型に応じて`value`の型が自動的に判定されます（整数列には整数、浮動小数点数列には浮動小数点数、文字列列には文字列を受け付けます）。
