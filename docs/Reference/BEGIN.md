...
!!! info "API"
	...
	`BEGIN`を呼んだ時点で、**その関数自身**は終了します（`BEGIN`より下の行は実行されません）。ただし**呼び出し元の連鎖は中断されません** —— 呼び出し元はそのまま実行を続け、制御は順に戻っていき、関数スタックが空になった時点で初めて目標の状態へ遷移します（その際にエンジンが該当状態のイベント関数を呼び出します）。「ここで全体の流れが終わる」と解釈しないでください。`BEGIN`は`RESULT`も書き換えません。
