...
!!! info "API"
    ...
    ```
	`FOR～NEXT` is an enhanced version of [`REPEAT～REND`](./REPEAT.en.md).  
	...
	Both are constructs that repeat X times, and support [`CONTINUE`](./CONTINUE.en.md) and [`BREAK`](./CONTINUE.en.md) within the loop.  
	...
	When `value` is 0, it becomes an infinite loop that repeats forever until a [`BREAK`](./CONTINUE.en.md) statement is executed.  
	...
	NEXT  
    ```
	Note that if you enter `FOR～NEXT` directly via commands like [`GOTO`](./GOTO.en.md), it executes normally up to just before `NEXT`, similar to `REPEAT～REND`, then ignores `NEXT` and continues from the next line.
