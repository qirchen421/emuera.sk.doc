...
!!! info "API"
	...
	当前文字颜色可通过[`GETCOLOR`](./GETCOLOR.zh.md)获取，默认文字颜色可通过[`GETDEFCOLOR`](./GETCOLOR.zh.md)获取。  
	自1.731版本起，`SETCOLOR`支持使用`0xRRGGBB`格式指定颜色。  
	如需使用颜色名称指定，请使用[`SETCOLORBYNAME`](./SETCOLORBYNAME.zh.md)。
...
这两行代码具有相同的效果。通过 [`GETCOLOR`](./GETCOLOR.zh.md) 命令获取到的颜色值将是后一种格式。
...
### 相关项目
- [SETBGMCOLOR](SETBGCOLOR.zh.md)
- [SETCOLORBYNAME](SETCOLORBYNAME.zh.md)